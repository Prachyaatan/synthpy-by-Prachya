...
class prep_data:
    def __init__(self, df, year, country, target, control,
        ...
        #X1X0_train = np.hstack([X0_train, X1_train.reshape(-1,1)])
        #X0_train = (X0_train.T)/np.std(X1X0_train, axis = 1)
        #X1_train = (X1_train.T)/np.std(X1X0_train, axis = 1)  
...
class synth:
    def __init__(self, PD, custom_V = [], method = "Nelder-Mead"): # tried opt: L-BFGS-B
        if len(custom_V) == 0:
            ini = (np.ones(PD.X0_train.shape[1])/PD.X0_train.shape[1]).reshape(-1,1).T
            #cons = ({'type': 'eq', 'fun': lambda x:  1 - sum(x)}) #sum to one
            bnds = tuple((0,None) for x in ini) #positive only
...
def loo(df, year, country, target, control, begin, train, validation, treatment, outcome,
        cov, use_Vbaseline = True, opt_method = "Nelder-Mead", Legend_loc = "lower left" , pred_method = "mean"):
...
def placebo_permu(df, year, country, target, control, 
                  begin, train, validation, treatment, 
                  outcome, cov,  placebo_start, use_Vbaseline = True, pb_pret_split = 0.5,
                  opt_method = "Nelder-Mead", pred_method = "mean", alpha = 0.05):
    ...
    for y in (np.arange(placebo_start, treatment, 1)): #year prior to actual treat
        for c in (control): #control units
            ...
            placebo_validation = int(train + int(pb_pret_split*(y - train)))#splot train-val period
